...
DURUM UZAYI (40 boyut):
  36 teknik feature + 4 portföy durumu
...
class NeuralNetwork:
    ...
    def backward(self, grad_out: np.ndarray) -> List[Dict]:
        """
        ...
        for i in reversed(range(n_layers)):
            inp = self._cache["inputs"][i]
            W   = self.layers[i]["W"]
            dW  = np.outer(inp, delta)
            db  = delta.copy()
            grads.insert(0, {"W": dW, "b": db})
            if i > 0:
                delta = (delta @ W.T) * (1 - self._cache["inputs"][i]**2)  # tanh'ın türevi
...
class TradingEnvironment:
    ...
    State: [36 feature + position_type(normalized) + unrealized_pnl + age + max_drawdown]
    Action: 0=FLAT, 1=LONG_S(5x), 2=LONG_L(10x), 3=SHORT_S(5x), 4=SHORT_L(10x)
    Reward: tanh(sharpe_step * 100)
    """
    N_ACTIONS   = 5
    STATE_DIM   = 40      # 36 feature + 4 portföy
    FEE_RATE    = 0.0006
    MARGIN      = 100.0   # USD
    ...
    def reset(self, features: Optional[np.ndarray] = None,
        ...
        """Yeni episode başlat."""
        self._features  = features  # (T, 36) feature matrisi
        self._prices    = prices    # (T,) fiyat serisi
        self._step      = 0
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        # ── BEKLEME ÖDÜLÜ ─────────────────────────────────────────────────────
        # FLAT kalmak için ceza — agent'ı harekete zorla
        if self._position == 0 and action == 0:
            reward -= 0.02  # FIX: 0.001 → 0.02 (FLAT local minimum önleme)
        ...
        return next_state, float(np.clip(reward, -2.0, 2.0)), done, info
...
class PPOAgent:
    ...
    def train_on_history(self,
        ...
        features_list: Her pair için (T, 36) feature matrisi listesi
