...
class TradingEnvironment:
    ...
    N_ACTIONS   = 5
    FEE_RATE    = 0.0006
    MARGIN      = 25.0    # USD (5000$ kasa için ~%0.5 risk/birim)
    ACTION_LEV  = {0: 0, 1: 5, 2: 10, 3: 5, 4: 10}
    ACTION_SIDE = {0: "FLAT", 1: "LONG", 2: "LONG", 3: "SHORT", 4: "SHORT"}

    def __init__(self, episode_length: int = 200):  # Daha uzun episod, uzun vadeli düşünme
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        if self._position != 0:
            if side == "FLAT" or \
               ...
               (side == "SHORT" and self._position > 0):
                ...
                # Ödül: çıkış PnL + maliyet (asimetrik: zararı daha çok cezalandır)
                reward += np.tanh(net_pnl / 2.0) if net_pnl > 0 else np.tanh(net_pnl / 1.5)
        ...
        # ── BEKLEME ÖDÜLÜ ─────────────────────────────────────────────────────
        # FLAT kalmak cezasız — sadece iyi fırsatta trade aç

        # ── ADIM İLERLE ──────────────────────────────────────────────────────
        self._step += 1
        done = (self._step >= min(self.episode_length, len(self._prices) - 1))

        # Episode sonu bonusu: Sharpe ratio (daha güçlü)
        ...
        return next_state, float(np.clip(reward, -3.0, 5.0)), done, info  # Daha geniş aralık
...
class PPOAgent:
    ...
    def __init__(self,
                 ...
                 n_epochs:     int   = 4,
                 batch_size:   int   = 256,
                 entropy_coef: float = 0.15,  # FIX: 0.01 → 0.05 → 0.15 (daha fazla keşif, SHORT bias için)
                 vf_coef:      float = 0.5):
