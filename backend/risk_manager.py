...
class RiskManager:
    ...
    # ── VOLATILITE POSIZYON AYARLAMA ───────────────────────────────────────

    def _estimate_volatility(self, symbol: str, scanner_cache: dict) -> float:
        """Pair'in anlik volatilitesini scanner verisinden tahmin et."""
        data = scanner_cache.get(symbol, {})
        change24 = abs(data.get("change_24h", 0) or 0)
        atr_norm = data.get("atr_normalized", 0) or 0

        # Volatility score (0.0 = dusuk, 1.0 = cok yuksek)
        vol = max(change24 / 15.0, atr_norm * 5)
        return min(vol, 1.0)

    def get_vol_adjusted_size(self, base_size: float, volatility: float) -> float:
        """Volatiliteye gore pozisyon buyuklugunu ayarla.
        Yuksek vol = kucuk pozisyon, dusuk vol = buyuk pozisyon.
        """
        # Vol 0.0 -> 1.5x, vol 0.5 -> 1.0x, vol 1.0 -> 0.5x
        vol_mult = 1.5 - volatility
        vol_mult = max(0.5, min(vol_mult, 1.5))
        return round(base_size * vol_mult, 2)

    # ── POZISYON BUYUKLUGU (KELLY CRITERION + VOLATILITE) ─────────────────
    ...
    def calculate_position_size(
        ...
        portfolio: dict,
        agent_states: dict,
        scanner_cache: dict = None,
    ) -> float:
        """
        Kelly Criterion + volatilite bazli pozisyon buyuklugu.
        ...
        Nihai buyukluk = capital * kelly_fraction * Kelly% * vol_multiplier
        """
        ...
        # Volatilite ayari
        if scanner_cache:
            vol = self._estimate_volatility(symbol, scanner_cache)
            kelly_pct *= self.get_vol_adjusted_size(1.0, vol)
