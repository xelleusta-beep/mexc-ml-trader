...
class TradingConfig:
    ...
    base_position_size: float = _env("BASE_POSITION_SIZE", 5.0)   # 5000$ kasa için
    ...
    min_confidence_rl_only: float = _env("MIN_CONFIDENCE_RL_ONLY", 62.0)  # RL tek başına daha düşük eşik
...
class RLConfig:
    ...
    entropy_coef: float = _env("RL_ENTROPY_COEF", 0.15)  # FIX: 0.05→0.15, daha fazla keşif
    vf_coef: float = _env("RL_VF_COEF", 0.5)

    # Environment
    episode_length: int = _env("RL_EPISODE_LENGTH", 200)
    margin: float = _env("RL_MARGIN", 25.0)
    ...
    v2_n_attn_heads: int = _env("V2_N_ATTN_HEADS", 4)
