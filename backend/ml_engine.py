...
class Indicators:
    ...
    def atr_percentile(h, lo, c):
        ...
        hist = [Indicators.atr(h[:i], lo[:i], c[:i], 14)
                for i in range(max(20, len(c)-20), len(c))]
...
class FeatureBuilder:
    ...
    def build_dataset(cls, klines, lookahead=5, threshold=0.005):
        ...
        if len(c) >= 50:
            ...
            dyn_thr  = float(np.clip(avg_move * 0.8, 0.002, 0.008))
        else:
            dyn_thr = threshold
...
def balance_classes(X: np.ndarray, y: np.ndarray, target_ratio: float = 0.35) -> tuple:
    ...
    if long_cnt > 0 and short_cnt > 0:
        ...
        # Eğer fark %50'den fazlaysa simetri uygula
        if max(long_cnt, short_cnt) > sym_target * 1.5:
...
# ─────────────────────────────────────────────────────────────────────────────
class MLEngine:
    RETRAIN_INTERVAL_HOURS    = 8  # FIX: 6→8 saat, daha az overfitting
    ...
    def __init__(self, model_dir="."):
        ...
        logger.info("MLEngine v2.2 — Tüm düzeltmeler aktif")
    ...
    def should_retrain(self):
        """
        FIX: feedback<20 durumunda _last_drift_retrain güncellenir (cooldown başlar).
        FIX: record_drift_event() sadece burada çağrılır.
        """
    ...
    def predict(self, symbol, klines, current_price):
        try:
            ...
            # Conf eşiği: %52 (daha fazla işlem)
            if conf < 52: sig = "WAIT"  # FIX: 55→52, WF %52 ile minimum işlem
            # FIX #5: 10x için %75+ conf
    ...
    def get_info(self):
        return {
            ...
            "label_method":  "lookahead=5bars, ATR-dynamic threshold",
