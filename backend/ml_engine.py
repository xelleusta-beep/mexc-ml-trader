"""
MEXC ML Trading System — Advanced ML Engine v2.1
================================================
...
✅ Drift Detection — FIX: is_drifting() artık saf fonksiyon (side-effect yok)
✅ FIX: Drift cooldown feedback<20 durumunda bile güncelleniyor
✅ FIX: Drift threshold yumuşatıldı (sigma 1.5→2.5, absolute drop 15%→25%)
...
class FeatureBuilder:
    ...
    def build_dataset(cls, klines, lookahead=5, threshold=0.005):
        ...
        # Dinamik threshold: varlığın gerçek volatilitesine göre ayarla
        # BTC/ETH gibi büyük cap pairlerde %0.5 çok yüksek → neredeyse tüm bar HOLD
        # ATR bazlı eşik: son 50 barın ortalama hareketinin yarısı, min %0.2 max %0.8
        if len(c) >= 50:
            avg_move = float(np.mean(np.abs(np.diff(c[-50:])) / (c[-50:-1] + 1e-10)))
            dyn_threshold = float(np.clip(avg_move * 0.8, 0.002, 0.008))
        else:
            dyn_threshold = threshold  # Yeterli veri yoksa varsayılan
        ...
        for i in range(MIN, len(c)-lookahead):
            ...
            label=1 if ret>dyn_threshold else (-1 if ret<-dyn_threshold else 0)
...
# DRIFT DETECTOR — v2.1
# DÜZELTMELER:
#   - is_drifting() artık PURE FUNCTION — drift_count'u asla artırmaz
#   - drift_count sadece MLEngine.should_retrain() içinde, karar verildiğinde artırılır
#   - Threshold yumuşatıldı: sigma 1.5→2.5, absolute_drop 15%→25%
#   - record_drift_event() yeni metod: sadece gerçek retrain kararında çağrılır
# ─────────────────────────────────────────────────────────────────────────────
class DriftDetector:
    def __init__(self, window=50, threshold_sigma=2.5):
        self._window = window
        self._threshold = threshold_sigma        # 1.5 → 2.5 (daha az hassas)
        self._recent = deque(maxlen=window)
        self._baseline_acc = 0.0
        self._drift_count = 0                    # Sadece record_drift_event() artırır

    def update(self, correct: bool):
        """Trade kapatıldığında çağrılır — sadece gerçek sonuç"""
    ...
    def is_drifting(self) -> bool:
        """
        PURE FUNCTION — hiçbir state değiştirmez, sadece okur.
        drift_count burada ARTMAZ.

        NOT: Baseline WF accuracy üzerinden hesaplanıyor (HOLD-ağırlıklı).
        Gerçek trade win_rate genellikle çok daha düşük olduğundan
        absolute_drop eşiği yüksek tutuldu (%45) — kalıcı drift önleme.
        """
        if len(self._recent) < 20 or self._baseline_acc < 0.01:
            return False
        recent_acc = float(np.mean(self._recent))

        # Absolute drop: baseline'dan %45+ düşüş (önceki: %25)
        # WF accuracy HOLD-label ağırlığından dolayı şişirilmiş olabiliyor;
        # gerçek trade win_rate doğal olarak daha düşük → eşik yüksek tutuldu
        absolute_drop = (self._baseline_acc - recent_acc) / (self._baseline_acc + 1e-10)
        if absolute_drop >= 0.55:
            return True

        # Sigma-based: standart sapma 3.0 (çok gevşek — sadece aşırı düşüşlerde tetiklenir)
    ...
    def record_drift_event(self):
        """
        Sadece MLEngine.should_retrain() gerçekten drift kararı verdiğinde çağrılır.
        is_drifting()'den AYRI tutulur.
        """
        self._drift_count += 1
    ...
    def get_status(self) -> dict:
        """get_status() is_drifting()'i bir kez çağırır — side effect yok"""
        return {
...
class MLEngine:
    ...
    # Drift cooldown artırıldı: 3sa → 6sa (spam önleme)
    DRIFT_RETRAIN_COOLDOWN_MIN = 360
    MIN_SAMPLES = 80
    ...
    def __init__(self, model_dir="."):
        ...
        logger.info("MLEngine v2.1 — Drift fix + 36 feature hazır")
    ...
    def train(self, klines, symbol="GLOBAL", feedback_boost=False):
        ...
        try:
            ...
            logger.info(f"✅ Eğitim [{symbol}] | {len(X)} sample | "
                        f"test_acc={acc:.1f}% | wf_acc={wf_acc}% | {elapsed:.1f}s")
            return rec
        finally:
            self._training_in_progress = False
    ...
    def train_on_multi_pair(self, klines_list, symbol="GLOBAL"):
        ...
        try:
            ...
            logger.info(f"Multi-pair class denge: {dict(zip(unique,cnt))} "
                        f"(orig: {dict(zip(unique_orig,cnt_orig))})")
            ...
            logger.info(f"✅ Multi-pair | {len(Xall)} sample, {len(all_X)} pair | "
                        f"wf_acc={wf_acc}% | {elapsed:.1f}s")
            return rec
        finally:
            self._training_in_progress = False
    ...
    def run_backtest(self, klines):
        ...
        logger.info(f"Backtest → ROI:{self._bt['roi']}% "
                    f"WR:{self._bt['win_rate']}% Sharpe:{self._bt['sharpe']}")
        return self._bt
    ...
    def record_trade_result(self, features, side, net_pnl, symbol=""):
        ...
        # Drift window'a ekle — kazanan trade doğru, kaybeden yanlış tahmin
        self.drift.update(net_pnl > 0)
        logger.debug(f"Trade result [{symbol}]: {side} | PnL=${net_pnl:.2f} | "
                     f"feedback={self.feedback.size()}")

    def should_retrain(self):
        """
        Retrain gerekip gerekmediğini kontrol eder.
        DÜZELTME:
          - Drift durumunda feedback < 20 olsa bile _last_drift_retrain güncellenir
            → cooldown devreye girer, sonsuz döngü olmaz
          - drift.record_drift_event() sadece burada, gerçek karar anında çağrılır
        """
        if self._training_in_progress:
            return False, ""
        ...
        if self.drift.is_drifting():
            mins_since_last = (time.time() - self._last_drift_retrain) / 60
            if mins_since_last >= self.DRIFT_RETRAIN_COOLDOWN_MIN:
                if self.feedback.size() >= 20:
                    # Gerçek retrain kararı → sayacı artır
                    self.drift.record_drift_event()
                    return True, "drift_detected"
                else:
                    # DÜZELTME: Feedback yetersiz ama cooldown'ı yine de başlat
                    logger.info(
                        f"Drift var ama feedback yetersiz ({self.feedback.size()}<20) "
                        f"— retrain ertelendi, cooldown başlatıldı"
                    )
                    self._last_drift_retrain = time.time()  # ← KRİTİK FIX
    ...
    def check_and_retrain(self, klines_getter):
        ...
        if reason == "drift_detected":
            self._last_drift_retrain = time.time()
            self._retrain_triggers["drift"] += 1
        elif reason.startswith("temporal"):
            self._retrain_triggers["temporal"] += 1
        elif reason.startswith("feedback"):
            self._retrain_triggers["feedback"] += 1
        return self.train(klines, symbol="AUTO_RETRAIN",
                          feedback_boost=self.feedback.size() >= 50)
    ...
    def predict(self, symbol, klines, current_price):
        try:
            ...
            # FIX: Eşik %60→%55 — çok az sinyal üretiliyordu
            if conf < 55: sig = "WAIT"
            # Kaldıraç eşikleri: yüksek kaldıraç için yüksek güven şartı
            # 15x: %82+, 10x: %75+ (önceden %72 idi — çok düşüktü), 5x: diğerleri
            lev = 15 if conf > 82 else 10 if conf > 75 else 5
            self._pred_count += 1
            return {
                "signal": sig, "confidence": conf,
                "indicators": self._active_inds(klines)[:4],
                "model": "GBM+RF v2.1", "leverage": lev,
    ...
    def load(self, path):
        ...
        try:
            ...
            self.rf._backend = payload["rf_backend"]; self.rf.clf = payload["rf_clf"]
            ...
            self._retrain_triggers = payload.get("retrain_triggers",
                {"temporal":0,"drift":0,"feedback":0,"manual":0})
            # Kayıtlı drift_count'u geri yükle (eski formatlarda 0)
            self.drift._drift_count = payload.get("drift_count", 0)
    ...
    def get_feature_importance(self):
        ...
        if imp is not None and len(imp) == len(names):
            return [{"name":n,"type":types[i],"importance":round(float(imp[i]*100),1)}
        ...
        return [{"name":n,"type":types[i],"importance":round(static[i]*100,1)}
                for i,n in enumerate(names)]
    ...
    def get_info(self):
        return {
            ...
            "feature_engineering_status": "Production v2.1 (36 features)",
            "data_processing": "Online Min15 Stream (limit=300)",
            "label_method":    "lookahead=5bars, threshold=0.5%",
