...
async def auto_train_on_startup():
    """Yedek: _initial_train_waiter çalışmazsa bu devreye girer."""
    try:
        await asyncio.sleep(90)  # Daha kısa bekleme
        if ml_engine._trained:
            return
        # Scanner çalışıyor mu kontrol et
        if len(scanner_cache) < 5 and len(_klines_cache) < 5:
            logger.info("auto_train_on_startup: henüz yeterli veri yok, 60sn daha bekleniyor")
            await asyncio.sleep(60)
            if ml_engine._trained:
                return
        logger.info("🤖 auto_train_on_startup: cache ve fallback pair'lerle eğitim")
        klines_list = []
...
def _log_config_status():
    ...
    logger.info("═══════════════════════════════════════")
    logger.info("  MEXC RL+ML Trader v3.0 — Konfigürasyon")
    logger.info("═══════════════════════════════════════")
    # API
    ...
    # ML
    logger.info(f"  ML Features: {config.ml.n_features} (v2: {config.ml.use_v2_features})")
...
async def auth_middleware(request: Request, call_next):
    # Public endpoints
    public_paths = {"/", "/api/status", "/api/pnl_timeline", "/api/health",
                    "/sw.js", "/manifest.json", "/ml-details"}
...
async def run_custom_backtest(symbol: str = "", sl_pct: float = 0.022,
    ...
    klines = _klines_cache.get(symbol, np.array([]))
    if len(klines) < 10:
        try:
            client = await get_http_client()
    ...
    if len(klines) < 10:
        return {"success": False, "error": f"{symbol}: yetersiz veri ({len(klines)} mum)"}

    try:
        prices = np.array([float(k[4]) for k in klines if k[4] is not None], dtype=np.float64)
...
async def telegram_webhook(payload: dict):
    try:
        ...
        if msg == "/status":
            ...
            active_pos = sum(1 for s in active_trades.values() if s.get("symbol"))
