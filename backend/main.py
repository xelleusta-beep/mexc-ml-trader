...
async def process_pair(client: httpx.AsyncClient, symbol: str) -> dict:
    ...
    # FIX: Ticker'dan 0 fiyat gelince klines'ın son close fiyatını kullan
    # SOL/USDT ve benzeri pairlerde lastPrice bazen 0 dönüyor
    MIN_PRICE = 0.000001
    if price < MIN_PRICE and klines is not None:
        try:
            fallback_price = float(klines["close"][-1])
            if fallback_price > MIN_PRICE:
                price = fallback_price
                logger.debug(f"[{symbol}] Ticker fiyatı 0 — klines fallback: {price}")
    ...
    # Yüksek kaldıraç = daha sıkı SL (hesap koruması)
    lev = max(1, prediction.get("leverage", 10))
    if lev >= 10:
        sl_pct = 0.012   # 10x-15x: %1.2 SL (sıkı)
        tp_pct = 0.030   # %3.0 TP  → R:R 1:2.5
    elif lev >= 7:
        sl_pct = 0.018   # 7-9x: %1.8 SL
        tp_pct = 0.040   # %4.0 TP
    else:
        sl_pct = 0.022   # 5x: %2.2 SL
        tp_pct = 0.050   # %5.0 TP → R:R 1:2.3
    ...
    # FIX: Eşikler gevşetildi — eski: %70/%72, yeni: %62/%62
    # Çok katı filtre neredeyse sıfır sinyal üretmesine yol açıyordu
    signals_agree = (
        (gbm_sig == rf_sig == main_sig) or           # Her ikisi hemfikir → kesin giriş
        (gbm_sig == main_sig and conf_val >= 62) or  # GBM hemfikir + %62 conf
        (rf_sig  == main_sig and conf_val >= 62)     # RF hemfikir + %62 conf
    )
    ...
    if can_enter and st["last_exit_time"]:
        # Zarar sonrası cooldown: kazanç=5dk, kayıp=2sa bekle (flip riskini önle)
        last_pnl = st.get("last_trade_pnl", 0)
        cooldown_secs = 7200 if last_pnl < 0 else 300
        if (datetime.utcnow() - st["last_exit_time"]).total_seconds() < cooldown_secs:
            can_enter = False
    if can_enter:
    ...
    if st["active_pos"] is not None:
        ...
        # Geçersiz/sıfır price → PnL güncelleme; TP/SL tetiklenmesin
            ...
            # Maksimum pozisyon süresi: 8 saat (sonsuz beklemeyi önle)
            MAX_POS_HOURS = 8
            pos_age_hours = (datetime.utcnow() - pos["entry_time_raw"]).total_seconds() / 3600
            if not close_pos and pos_age_hours >= MAX_POS_HOURS:
                close_pos = True
                exit_reason = f"⏰ SÜRE DOLDU ({pos_age_hours:.1f}sa)"
                logger.info(f"[{symbol}] Maksimum süre aşıldı ({pos_age_hours:.1f}sa) — pozisyon kapatılıyor")
    ...
    if close_pos and st["active_pos"] is not None:
        ...
        st["last_trade_pnl"] = round(net_pnl, 2)   # Cooldown için kaydediyoruz
        save_data()
