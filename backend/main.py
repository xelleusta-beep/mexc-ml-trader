...
async def process_pair(client: httpx.AsyncClient, symbol: str) -> dict:
    ...
    if prediction["signal"] == "SHORT":
        sl = round(price * 1.025, 10); tp = round(price * 0.955, 10)
    else:
        sl = round(price * 0.975, 10); tp = round(price * 1.045, 10)
    ...
    if st["active_pos"] is not None:
        pos = st["active_pos"]
        entry_p = pos["entry_price"]; side = pos["side"]
        lev = pos["leverage"];        size = pos["size"]
