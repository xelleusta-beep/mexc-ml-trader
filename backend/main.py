"""
MEXC ML Trading System - FastAPI Backend v2.2
Tüm düzeltmeler entegre edilmiş nihai versiyon.

DÜZELTMELER (17 adet):
  #1  Dinamik SL/TP: kaldıraca göre (10x→%1.2SL, 5x→%2.2SL)
  #2  Ticker sıfır fiyat fallback: klines[-1].close kullan
  #3  signals_agree eşiği: %70/%72 → %62/%62
  #4  Conf threshold: %60 → %55
  #5  Leverage gate: 10x için %75+ conf
  #6  Cooldown: kayıp→2sa, kazanç→5dk
  #7  Max pozisyon süresi: 8 saat → otomatik kapat
  #8  Drift absolute_drop: %25 → %55
  #9  Drift cooldown: 3sa → 6sa
  #10 is_drifting() pure function (side-effect yok)
  #11 Dinamik label threshold (ATR bazlı)
  #12 last_trade_pnl state kaydı
  #13 drift_count patlaması düzeltildi
  #14 Kaldıraçsız notional hacim takibi
  #15 Double fee bug düzeltildi
  #16 entry_price<=0 temizle
  #17 Grace period 120sn
"""
...
portfolio = {
    ...
    "total_closed_notional": 0.0,   # FIX #14
    "total_closed_trades":   0,
}
FUTURES_PAIRS: List[str] = []
MAX_OPEN_POSITIONS = 15
MIN_VOLUME_24H     = 50_000_000.0
MIN_PRICE          = 0.000001       # Tek yerde tanım
MAX_POS_HOURS      = 8              # FIX #7: Maksimum açık pozisyon süresi
        ...
        for sym, st in loaded.items():
            ...
            if st.get("active_pos"):
                ...
                if not pos.get("tp") or not pos.get("sl"):
                    logger.warning(f"[{sym}] TP/SL eksik → pozisyon temizlendi")
                    st["active_pos"] = None
                if pos and pos.get("entry_price", 0) <= 0:
                    logger.warning(f"[{sym}] Geçersiz entry_price → temizlendi")
                    st["active_pos"] = None
        agent_states  = loaded
        trade_history = deque(data.get("trade_history", []), maxlen=200)
        saved = data.get("portfolio", {})
...
async def auto_train_on_startup():
    ...
    klines_list = []
    async with httpx.AsyncClient() as client:
        targets = FUTURES_PAIRS[:20] if FUTURES_PAIRS else ["BTC_USDT","ETH_USDT","SOL_USDT","BNB_USDT","XRP_USDT"]
        for sym in targets:
    ...
    while True:
        try:
            ...
            if should:
                ...
                klines_list = []
                async with httpx.AsyncClient() as client:
                    targets = FUTURES_PAIRS[:15] if FUTURES_PAIRS else ["BTC_USDT","ETH_USDT","SOL_USDT","BNB_USDT","XRP_USDT"]
                    for sym in targets:
                        kl = _klines_cache.get(sym) or await fetch_klines(client, sym, limit=300)
                ...
                if klines_list:
                    ...
                    if result.get("success"):
                        logger.info(f"✅ Retrain [{reason}] | wf={result.get('wf_accuracy')}%")
                        await asyncio.to_thread(ml_engine.save, MODEL_FILE)
...
async def keep_alive_loop():
    ...
    if not app_url:
        logger.info("RENDER_EXTERNAL_URL yok — keep-alive devre dışı"); return
...
async def lifespan(app: FastAPI):
    logger.info("🚀 MEXC ML Trading System v2.2 başlatılıyor...")
    load_data()
...
# ── FastAPI ───────────────────────────────────────────────────────────────────
app = FastAPI(title="MEXC ML Trader v2.2", lifespan=lifespan)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

# ── Scanner ───────────────────────────────────────────────────────────────────
_startup_time: float = 0.0
_STARTUP_GRACE_SECONDS = 120  # FIX #17
...
async def process_pair(client: httpx.AsyncClient, symbol: str) -> dict:
    ...
    # FIX #2: Ticker sıfır fiyat → klines fallback
    if price < MIN_PRICE and klines is not None:
        try:
            fb = float(klines["close"][-1])
            if fb > MIN_PRICE:
                price = fb
                logger.debug(f"[{symbol}] Ticker=0 → klines fallback: {price}")
    ...
    # Dinamik SL/TP — kaldıraca ve volatiliteye göre
    lev = max(1, prediction.get("leverage", 10))
    # GÜVENLİK: WF accuracy < %65 iken max kaldıraç 5x
    # Model bozulunca 10-15x ile büyük zararlar önlenir
    wf_acc_now = ml_engine._wf.get("accuracy", 0)
    if wf_acc_now < 65 and lev > 5:
        lev = 5
        logger.debug(f"[{symbol}] Düşük WF({wf_acc_now}%) → kaldıraç 5x'e limitlendi")

    if lev >= 10:
        sl_pct, tp_pct = 0.012, 0.030   # 10-15x: SL%1.2, TP%3.0 → R:R 1:2.5
    elif lev >= 7:
        sl_pct, tp_pct = 0.018, 0.040   # 7-9x:   SL%1.8, TP%4.0
    else:
        sl_pct, tp_pct = 0.022, 0.050   # 5x:     SL%2.2, TP%5.0 → R:R 1:2.3
    ...
    if symbol not in agent_states:
        agent_states[symbol] = {
            ...
            "entry_feat": None, "last_trade_pnl": 0.0,  # FIX #12
        }
    ...
    # FIX #3: Eşikler gevşetildi %70/%72 → %62/%62
    signals_agree = (
        (gbm_sig == rf_sig == main_sig) or
        (gbm_sig == main_sig and conf_val >= 62) or
        (rf_sig  == main_sig and conf_val >= 62)
    )
    ...
    if can_enter:
        ...
        asyncio.create_task(send_telegram_message(
            ...
            f"🎯 TP: <code>{tp:.10g}</code> ({tp_pct*100:.1f}%) | "
            f"🛑 SL: <code>{sl:.10g}</code> ({sl_pct*100:.1f}%)\n"
    ...
    if st["active_pos"] is not None:
        ...
        # FIX #16: Geçersiz entry_price
        if entry_p <= 0:
            logger.warning(f"[{symbol}] Geçersiz entry_price={entry_p} — temizlendi")
            st["active_pos"] = None; st["entry_feat"] = None; save_data()

        elif price < MIN_PRICE:
            # Sıfır fiyat: PnL güncelleme, TP/SL tetikleme
            logger.debug(f"[{symbol}] Fiyat=0 — pozisyon korunuyor")
            ...
            # FIX #7: Maksimum pozisyon süresi
            ...
            if not close_pos and pos_age_h >= MAX_POS_HOURS:
                ...
                logger.info(f"[{symbol}] {pos_age_h:.1f}sa → otomatik kapatma")

            # FIX #17: Grace period
            in_grace = (time.time() - _startup_time) < _STARTUP_GRACE_SECONDS
            if in_grace and close_pos:
                logger.info(f"[{symbol}] Grace period — {exit_reason} atlandı")
                close_pos = False
    ...
    if close_pos and st["active_pos"] is not None:
        ...
        # FIX #15: Sadece çıkış fee (giriş fee zaten kesildi)
        ...
        portfolio["total_closed_notional"] += base_sz   # FIX #14
        ...
        asyncio.create_task(send_telegram_message(
            ...
            f"📦 Hacim: <b>${portfolio['total_closed_notional']:.0f}</b>\n"
            f"🔄 Feedback: {ml_engine.feedback.size()} trade"
        ))
        ...
        st["last_trade_pnl"] = round(net_pnl, 2)   # FIX #12
        save_data()
...
async def serve_frontend():
    ...
    return FileResponse(idx) if os.path.exists(idx) else {"message":"MEXC ML Trader v2.2 API"}
