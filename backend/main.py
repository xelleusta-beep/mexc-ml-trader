...
MAX_OPEN_POSITIONS = 20  # FIX: 15→20
MIN_VOLUME_24H     = 100_000_000.0  # FIX: 50M→100M, daha likit pairler
MIN_PRICE          = 0.000001
MAX_POS_HOURS      = 0  # DEVRE DIŞI: zaman aşımı kapatma iptal edildi
...
def hybrid_predict(symbol: str, klines: Optional[dict],
    ...
    # Conf eşiği — daha az ama kaliteli işlem
    if final_conf < 58:
        final_sig = "WAIT"  # FIX: 55→58
...
async def process_pair(client, symbol: str) -> dict:
    ...
    # FIX: Asimetrik SL/TP — dar SL, geniş TP (3:1 R/R hedefi)
    if lev >= 10:
        sl_pct, tp_pct = 0.010, 0.035   # 1:3.5 R/R
    elif lev >= 7:
        sl_pct, tp_pct = 0.015, 0.045   # 1:3.0 R/R
    else:
        sl_pct, tp_pct = 0.018, 0.054   # 1:3.0 R/R
    ...
    signals_agree = (
        (source == "RL+ML" and gbm_rf_agree) or                    # RL+ML + GBM+RF
        (source == "ML_only" and conf_val >= 62 and gbm_rf_agree) or  # Yüksek conf + GBM+RF
        (source == "RL_only" and conf_val >= 68) or                # Sadece RL, çok yüksek
        (gbm_rf_agree and conf_val >= 60)                          # GBM+RF hemfikir + conf
    )
    ...
    if can_enter and st["last_exit_time"]:
        ...
        cooldown_sec = 3600 if last_pnl < 0 else 180  # FIX: 7200→3600s, 300→180s
